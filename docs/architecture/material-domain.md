...
## Workers and observability (implemented)

```text
WorkerRun    uuid, type (registry key), status (queued | running | succeeded | failed),
             subject (material | variant | representation), payload, result, error,
             attempts, actor, requesting tenant, queue, queued/started/finished, duration
TrackedJob   abstract queued job: launch() records the run and dispatches; handle() marks
             running → succeeded/failed; requeue() repeats a run; NotTenantAware
FileAccess   every read of a library file: channel (web | prismfs), user or principal,
             path, ip, user agent, bytes, time
```

- Queue: Redis under Laravel Horizon (`php artisan horizon` is the queue
  service; dashboard at `/horizon` for super-admins). The business view is
  `/jobs`: every run, its subject, result or error, duration, who asked, and
  a retry for failures. Provenance events carry the run's uuid as `job_id`,
  so a file's lineage names the run that produced it.
- Job types are registered in `config/opal.php` (`workers`), so a run can be
  re-queued from its record. New work (upscaling, scraping, rendering) is a
  new `TrackedJob` subclass plus a registry entry.
- `DownscaleRepresentation` is the first worker: it resizes a representation's
  image maps to lower quality tiers (Imagick Lanczos, GD fallback), records a
  `downscaled` provenance event, and approves the result when the source was
  approved, since the transform is deterministic. `opal:tiers:backfill` queues
  it for every approved canonical set missing tiers; the material record has a
  "Generate tiers" action per variant.
- Web downloads are recorded in `file_accesses`. PrismFS emits structured audit
  lines per operation (request id, tenant, principal uid, path, result); an
  endpoint to ingest those into the same table is the next step.

## JSON API (implemented)

`/api/v1` with Sanctum bearer tokens issued from Settings → API tokens.
Read-only for now: `GET materials` (search and filters, visibility-aware),
`GET materials/{code}` (variants, representations, file URLs; aliases accepted),
`GET variants/{code}`, `GET variants/resolve?platform=&reference=` (a Revit id,
name, or any string containing a variant code). Documentation is generated by
Scramble from the routes and resources and rendered with Stoplight Elements at
`/docs/api` (`/docs/api.json` is the OpenAPI document).
...
- `Host` is the client application. `RevitHost` (in the extension's `lib`)
  edits appearance assets and material parameters; `UnixHost` keeps a JSON
  project so the same workflow runs in a shell.
- `unix_proxy.sh` mounts a drive with PrismFS from the control-plane
  manifest and runs the loop end to end; `opal_cli.py` exposes the operations
  individually.

Endpoints added for the client: `GET /api/v1/drives`,
`GET /api/v1/variants/{code}/paths?drive=slug` (the drive's paths, target,
quality, role and hash for the variant's published files) and
`POST /api/v1/variants/{code}/identities` (identity write-back; requires a
workspace membership or super-admin).
...
three.js (`MeshPhysicalMaterial` under a room environment light) on a
sphere, plane or cube, tiled by the variant's real-world size. It shows what
