...
Render's `olsyn-access` secret holds `RENDER_ACCESS_TOKEN` and `RENDER_SESSION_SECRET`; its deployment pins the client ID and `RENDER_OLSYN_ACCESS_ENABLED=true`. Main's local grants remain authoritative for the existing Cloudflare identity too. `/auth/recovery` clears an expired WorkOS session, validates the existing Cloudflare assertion and establishes its own expiring service session. The existing Cloudflare Access application is scoped to `render.olsyn.com/auth/recovery`; normal login uses WorkOS directly. Keep its original audience and policies. Move this edge gate only after the new app authorization is verified. Workers keep their current protocol, so no running GPU jobs need replacement.
