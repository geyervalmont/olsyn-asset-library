...
beforeEach(function () {
    ...
    $this->releaseManifest = [
        ...
        'package' => ['name' => 'OPAL-Revit-Package.zip', 'sha256' => str_repeat('b', 64), 'bytes' => 1_500_000],
    ];
...
test('the native Revit installer is downloadable from settings', function () {
    $this->actingAs($this->user)
        ...
        ->assertOk()
        ->assertSee('OPAL for Revit 2027')
        ->assertSee('Version 0.1.0.7')
        ->assertSee('data-test="download-revit"', false)
        ->assertSee(route('revit.download', ['channel' => 'development', 'asset' => 'installer']), false);
});
...
test('clients discover updates through OPAL and downloads stay on an OPAL address', function () {
    ...
    expect($release['installer']['url'])->toBe(route('revit.download', ['channel' => 'development', 'asset' => 'installer']))
        ->and($release['package']['url'])->toBe(route('revit.download', ['channel' => 'development', 'asset' => 'package']));
