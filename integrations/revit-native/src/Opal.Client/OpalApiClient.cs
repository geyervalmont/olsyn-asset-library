...
public sealed class OpalApiClient : IDisposable
{
    ...
    public Task<ReleaseManifest> LatestReleaseAsync(string channel, CancellationToken cancellationToken = default) =>
        SendAsync<ReleaseManifest>(HttpMethod.Get, $"api/v1/client-releases/revit/{Uri.EscapeDataString(channel)}", null, dataEnvelope: false, cancellationToken);
